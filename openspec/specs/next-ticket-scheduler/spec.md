...
The skill SHALL output the recommended ticket's ID, title, and a one-to-two sentence explanation of why it was chosen. The output SHALL include a single `/project-management start <id>` call-to-action instead of the previous two-line manual hint. When the recommendation comes from a sibling repo, the source repo SHALL be identified in the output.
