...
On `wiki init`, the skill SHALL create the full wiki directory tree under `wiki.root` as defined in `vibe_wiki.config.yaml`: `concepts/`, `sources/`, `authors/`, `tools/`, `workflows/`, `terms/`, `raw/`, `.state/`. It SHALL copy starter files (`index.md`, `log.md`) if they do not already exist. It SHALL NOT overwrite existing files.
...
- **THEN** the skill creates all subdirectories, copies `index.md` and `log.md` from `assets/starter/`, creates `raw/` as an empty directory, and confirms each directory created
